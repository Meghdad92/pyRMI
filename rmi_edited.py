...
class AttributeData(T.utils.data.Dataset):
  def __init__(self, src_file=None, att=None, m_rows=None):
    ...
    stats_df = df.groupby(self.att)[self.att].agg('count').pipe(pd.DataFrame).rename(columns = {att: 'frequency'})
  ...
  def newdata(self,loop="all",lb=100, hb=200,updated_file=None):
    if updated_file == None:
      df_base = pd.read_csv(self.src_file, sep=',',usecols=[self.att])
      df1 = df_base[df_base[self.att] > lb]
      df2 = df1[df1[self.att] < hb]
      df_new = df2.sample(frac=0.5,replace=True)


      df = pd.concat([df_base,df_new])

      df.to_csv("data/updated_data.csv",sep=',',index=None,header=True)
    else:
      df = pd.read_csv(updated_file, sep=',',usecols=[self.att])


    stats_df = df.groupby(self.att)[self.att].agg('count').pipe(pd.DataFrame).rename(columns = {self.att: 'frequency'})
    stats_df['pdf'] = stats_df['frequency'] / sum(stats_df['frequency'])
    stats_df['cdf'] = stats_df['pdf'].cumsum()
    stats_df = stats_df.reset_index()
    
    df_transfer = stats_df[stats_df[self.att] <= lb].sample(frac=0.2)
    df_update = stats_df[stats_df[self.att] > lb].sample(frac=1)
...
class RMI(Net):
  ...
  def update_single_model(self,model_idx,data_ldr,transfer_set_ldr, lamda=0.3, alpha=1/5):
      ...
      for epoch in range(0, max_epochs):
        ...
        if len(transfer_set_ldr) > len(data_ldr):
          for (X_tr, Y_tr), (X, Y) in (zip(transfer_set_ldr, cycle(data_ldr))):                   
            ...
            loss_val = 0.1*loss1 + 0.2*loss2 + 0.7*loss3
  ...
  def incremental_update(self,dataset, src_file, att):
      if dataset == "synthetic":
        ...
        train_ds = AttributeData(src_file=src_file, att=att)
        train_ds.newdata(loop="transferset",updated_file="data/updated_data.csv")
        transfer_ldr = T.utils.data.DataLoader(train_ds,batch_size=bat_size, shuffle=True)


        train_ds_total = AttributeData(src_file="data/updated_data.csv", att=att)
        data_ldr = T.utils.data.DataLoader(train_ds_total, batch_size=bat_size, shuffle=True)
      ...
      bat_size = 5#int(bat_size/self.B)
      print ("list",list(sub_X.keys()))
      os.sys.exit()
      for sub in range(1,self.B+1):
          ...
          """
          if sub in sub_X.keys() and sub in sub_X_tr.keys():
              sub_dataset = T.utils.data.TensorDataset(sub_X[sub],sub_Y[sub])
              sub_data_ldr = T.utils.data.DataLoader(sub_dataset,batch_size=bat_size, shuffle=True)
          ...
          if sub in sub_X.keys() and sub not in sub_X_tr.keys():              
              sub_dataset = T.utils.data.TensorDataset(sub_X[sub],sub_Y[sub])
              sub_data_ldr = T.utils.data.DataLoader(sub_dataset,batch_size=bat_size, shuffle=True)
              #self.train_single_model(sub, sub_data_ldr)
              self.finetune_single_model(sub, sub_data_ldr)

          if sub not in sub_X.keys():              
              pass
          """
  def predict_all(self,dataset, src_file, att):
      ...
      print ("\nrelative errors :")
      print ("total mean= {:0.2f}\t total median= {:0.2f}\t total 95th= {:0.2f}".format(np.mean(total_err),np.median(total_err),np.percentile(total_err,95)))         
...
def update():
  ...
  rmi.incremental_update(dataset="attributes", src_file="data/forest.csv",att="Aspect")

  with open('updated.pckl', 'wb') as f:
      pickle.dump(rmi,f)

def train(data, attribute):
  T.manual_seed(4)  # representative results 
  np.random.seed(4)


  rmi = RMI(input_size=1)
  rmi.train(dataset="attributes", src_file=data,att=attribute)

  with open('base.pckl', 'wb') as f:
      pickle.dump(rmi,f)

def test(model, data, attribute):

  rmi = None
  with open(model, 'rb') as f:
      rmi = pickle.load(f)

  rmi.predict_all(dataset="attributes", src_file=data,att=attribute) 

if __name__ == "__main__":
  #train(data='data/forest.csv',attribute='Aspect')
  update()
  #test(model="base.pckl",data="data/forest.csv", attribute="Aspect")
  test(model="updated.pckl",data="data/updated_data.csv", attribute="Aspect")
